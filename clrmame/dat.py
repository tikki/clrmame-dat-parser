...
def _parse(
    toks: Iterable[Token], seq_type: Seq
) -> Iterable[Tuple[Key, Union[SeqT, Value]]]:
    ...
    for tok in toks:
        if key is None:
            ...
            value: Union[SeqT, Value] = (
                tok if tok is not "(" else seq_type(_parse(toks, seq_type))
            )
            yield (key, value)
            key = None
...
def _struct_decoder(pairs: Iterable[DecoderT]) -> Iterable[DecoderT]:
    ...
    return tuple(
        (key, val if key not in typemap else typemap[key].from_pairs(val))
...
def parse(lines: Iterable[str]) -> Iterable[Union[AnyStruct, Value]]:
    ...
    return (
        val for key, val in _struct_decoder(_parse(_tokens(chars), _struct_decoder))
    )
